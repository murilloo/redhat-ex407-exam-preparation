#!/bin/bash

# Create Ansible user
adduser ansible

# Set password for Ansible
echo password | passwd ansible --stdin 

# Intall vim
sudo yum install -y vim

# Config Vim
echo "autocmd FileType yaml setlocal ai ts=2 sw=2 et" > /home/ansible/.vimrc
chown ansible:ansible /home/ansible/.vimrc

# Activating RHEL 7.5 repos
subscription-manager register --auto-attach

# Activate Ansible 2.7
subscription-manager repos --enable=rhel-7-server-ansible-2.7-rpms 

# Install Ansible
yum install ansible -y

# Setup /etc/hosts
cat << EOF >> /etc/hosts
192.168.122.100 control control.example.com
192.168.122.101 node1 node1.example.com
192.168.122.102 node2 node2.example.com
EOF
